...
/*
 * Simpilified shell from Professor.
 * Added authentication between client and server.
 * Parameters will now include clientID and clientPW for authentication purposes.
 */
int
RemoteShell(char *destination, int portN, char *clientID, char *clientPW)
{
	...
	/*Send the clientID to the server for authentication. */
	outchars = strlen(clientID);
	printf("Sending ID to server for authentication.\n");
	if ((n=write(sock, clientID, outchars))!=outchars)	/* send error */
	{
		printf("RemoteShell(%s, %d): has %d byte send when trying to send %d bytes to RemoteShell: `%s`\n", destination, portN, n, outchars, buf);
		close(sock);
		return -1;
	}
	/* Get result from the server if the ID is valid or not.*/
	...
	/*If ID is accepted, client will send the password to the server for authentication. */
	printf("Sending Password to server for authentication.\n");
	outchars = strlen(clientPW);
	if ((n=write(sock, clientPW, outchars))!=outchars)	/* send error */
	{
		printf("RemoteShell(%s, %d): has %d byte send when trying to send %d bytes to RemoteShell: `%s`\n", destination, portN, n, outchars, buf);
		close(sock);
		return -1;
	...
	/*If Authentication is successful, client-server connection is established. */
...
main(int argc, char *argv[])
{
	...
	/*Get the clientID and clientPW from input and pass to RemoteShell for authentication checking */
	clientID = argv[3];
	clientPW = argv[4];
	RemoteShell(destination, portN, clientID, clientPW);
	exit(0);
}
